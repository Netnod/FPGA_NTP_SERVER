...
class ApplicationConfig(object):
    def __init__(self, appname, debug=False):
    ...
    def conf_filename(self, value):
        ...
        if self.__conf_filename != value:
           raise Exception("Cannot set conf_filename twice! Currently: {}, New Value: {}".format(self.__conf_filename, value))
    ...
    def write_default_config(self):
        ...
        if config._defaults != self.default:
            config._defaults = self.default
    ...
    def update(self, key, value, ignore_is_shutdown=False):
        if self.__debug: print("{}: Updating {} to {}".format(self.__class__.__name__, key, value))
...
config = ApplicationConfig(appname)


def get_generic_cli_function(key, ftype="", _format_dict={}, hidden=False):
    format_dict = {"synopsis_key":key, "usage_key":key, "description":ftype+" "+_format_dict.get("description",""), "allowed_values":["STRING"], "appname":appname}
    ...
    """
        if format_dict["set_allowed_args"] not in ["STRING", "NUMBER", "NEGNUMBER", "FLOAT"]:
            if value not in format_dict["allowed_values"]:
                raise ValueError("Invalid argument \"{}\", valid arguments are: {}".format(value, format_dict["set_allowed_args"]))
    ...
    funcs = {"set"     : generic_set_function,
...
class ApplicationConfigItem(object):
    def __init__(self, key_regex, default, func=None, gen_cli=None, gen_hidden=False, debug=False):
        ...
        if gen_cli is not None:
            ...
            if debug: print("Creating CLI commands for key: {}..".format(self.__key_regex))
            for ftype in ["set", "show", "no", "default"]:
                if debug: print("Creating {} CLI command for key {}".format(ftype, key_regex))
    ...
    def __update(self, key, value, ignore_is_shutdown):
        if self.__debug: print("{}.__update: Updating {} to {}".format(self.__class__.__name__, key, value))
        if self.__func is not None:
            self.__func(config.app, key, value, ignore_is_shutdown)

    def match(self, key):
        m = re.match(self.__key_regex, key)
        if self.__debug: print("Checking if {} matches {} : {}".format(key, self.__key_regex, True if m else False))
        if m:
            return True
        return False
    ...
    def update(self, key, value, ignore_is_shutdown=False):
        if key is not None and self.match(key):
            if self.__debug: print("{}.update: Updating {} to {}".format(self.__class__.__name__, key, value))
    ...
    def __call__(self, func):
        if self.__debug: print("{}.__call__: key: {}, func={}".format(self.__class__.__name__, self.__key_regex, func.__name__))
        if self.__func is not None:
            raise RuntimeError("Call-back function for key {} has alread been registered: {}".format(self.__key_regex, self.__func.__name__))
        self.__func = func
        return func
